...
This uses the sample split files generated by `aam train` (via CLN-10) to ensure a fair comparison. Output includes R², MAE, and RMSE metrics printed to console, plus a TSV file with predictions in the same format as AAM's `val_predictions.tsv`.
...
1170 tests covering data pipeline, models, training, and end-to-end workflows.
