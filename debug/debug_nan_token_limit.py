...
def check_for_nan(tensor: torch.Tensor, name: str, step: str = "") -> bool:
    ...
    if has_nan or has_inf:
        ...
        if finite_mask.any():
            ...
            print(
                f"Finite values - min: {finite_values.min().item():.6f}, max: {finite_values.max().item():.6f}, mean: {finite_values.mean().item():.6f}"
            )
        else:
...
def debug_collate_fn(batch, token_limit, unifrac_distances, unifrac_metric):
    ...
    if num_asvs > 0:
        ...
        if invalid_tokens.any():
            ...
            print(
                f"  Invalid positions: batch={invalid_positions[0][:5].tolist()}, asv={invalid_positions[1][:5].tolist()}, pos={invalid_positions[2][:5].tolist()}"
            )
...
def debug_model_forward(model, tokens, step_name="forward"):
    ...
    pooled = model.sample_encoder.asv_encoder.attention_pooling(transformer_out, mask=mask)
