...
def create_visualizations(
    distance_matrix: DistanceMatrix, analysis_results: Dict, metadata_results: Dict, output_dir: Path
) -> None:
    ...
    if metadata_results and "counts_array" in metadata_results:
        ...
        if zero_sample_indices and non_zero_sample_indices:
            ...
            ax1.hist(
                non_zero_counts, bins=50, alpha=0.6, label="Samples without zero distances", color="blue", edgecolor="black"
            )
    ...
    if n_samples <= max_samples_for_heatmap:
        ...
        if HAS_SEABORN:
            sns.heatmap(distance_matrix.data, cmap="viridis", square=True, cbar_kws={"label": "UniFrac Distance"}, ax=ax)
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Investigate zero-distance UniFrac pairs in training data")
    parser.add_argument("--biom-path", type=str, required=True, help="Path to BIOM table file")
    parser.add_argument("--tree-path", type=str, required=True, help="Path to phylogenetic tree file (.nwk)")
    parser.add_argument("--rarefaction-depth", type=int, default=1000, help="Rarefaction depth for samples (default: 1000)")
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    rarefied_table, distance_matrix, sample_ids = load_data(str(biom_path), str(tree_path), args.rarefaction_depth)
