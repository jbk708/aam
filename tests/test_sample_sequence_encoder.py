"""Unit tests for SampleSequenceEncoder class."""
...
class TestSampleSequenceEncoder:
    """Test suite for SampleSequenceEncoder class."""

    def test_init(self, sample_encoder):
        """Test SampleSequenceEncoder initialization."""
        assert sample_encoder is not None
        assert isinstance(sample_encoder, nn.Module)

    def test_init_with_nucleotides(self, sample_encoder_with_nucleotides):
        """Test SampleSequenceEncoder initialization with nucleotide prediction."""
        assert sample_encoder_with_nucleotides is not None
    ...
    def test_forward_shape_embeddings_only(self, sample_encoder, sample_tokens):
        """Test forward pass output shape without nucleotide predictions."""
        result = sample_encoder(sample_tokens, return_nucleotides=False)
    ...
    def test_forward_full_length_sequences(self, sample_encoder, sample_tokens_full_length):
        """Test forward pass with full-length sequences."""
        result = sample_encoder(sample_tokens_full_length)
        assert result.shape == (2, 5, 64)
        assert not torch.isnan(result).any()
        assert not torch.isinf(result).any()

    def test_forward_with_partial_asv_masking(self, sample_encoder, sample_tokens_with_partial_asvs):
        """Test forward pass with partial ASV masking."""
        result = sample_encoder(sample_tokens_with_partial_asvs)
        assert result.shape == (2, 10, 64)
        assert not torch.isnan(result).any()
        assert not torch.isinf(result).any()

    def test_forward_inference_mode(self, sample_encoder, sample_tokens):
    ...
    def test_forward_training_mode_without_nucleotides(self, sample_encoder_with_nucleotides, sample_tokens):
        ...
        sample_encoder_with_nucleotides.train()
        result = sample_encoder_with_nucleotides(sample_tokens, return_nucleotides=False)
        assert isinstance(result, torch.Tensor)
        assert result.shape == (2, 10, 64)

    def test_forward_no_nucleotide_head(self, sample_encoder, sample_tokens):
        """Test that encoder without nucleotide head doesn't return predictions."""
        result = sample_encoder(sample_tokens, return_nucleotides=True)
        assert isinstance(result, torch.Tensor)
        assert result.shape == (2, 10, 64)

    def test_gradients_flow(self, sample_encoder, sample_tokens):
        """Test that gradients flow correctly."""
        result = sample_encoder(sample_tokens)
        loss = result.sum()
        loss.backward()

        for param in sample_encoder.parameters():
    ...
    def test_gradients_with_nucleotides(self, sample_encoder_with_nucleotides, sample_tokens):
        ...
        for param in sample_encoder_with_nucleotides.parameters():
            assert param.grad is not None
            assert not torch.isnan(param.grad).any()
            assert not torch.isinf(param.grad).any()

    def test_no_gradient_explosion(self, sample_encoder, sample_tokens):
        """Test that gradients don't explode."""
        result = sample_encoder(sample_tokens)
        loss = result.sum()
        loss.backward()

        max_grad = max(p.grad.abs().max().item() for p in sample_encoder.parameters() if p.grad is not None)
        assert max_grad < 1e6, f"Gradient explosion detected: max_grad={max_grad}"

    def test_no_gradient_vanishing(self, sample_encoder, sample_tokens):
    ...
    def test_forward_same_device(self, sample_encoder, sample_tokens):
    ...
    def test_dropout_training_mode(self, sample_encoder, sample_tokens):
        ...
        sample_encoder.train()
        result1 = sample_encoder(sample_tokens)
        result2 = sample_encoder(sample_tokens)
        assert not torch.allclose(result1, result2, atol=1e-5)

    def test_dropout_eval_mode(self, sample_encoder, sample_tokens):
    ...
    def test_forward_with_zero_padding(self, sample_encoder):
    ...
    def test_forward_variable_length_sequences(self, sample_encoder):
    ...
    def test_mask_creation_from_tokens(self, sample_encoder):
