...
def compute_distance_chunk(args: Tuple[List[str], Table, str, int, str]) -> Tuple[List[str], np.ndarray]:
    ...
    if metric == "unweighted":
        ...
        for i in range(len(chunks)):
            for j in range(i + 1, len(chunks)):
                ...
                cross_chunk_tasks.append((combined_ids, table, tree_path, threads_per_worker, metric))
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute UniFrac distances in parallel", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    ...
    parser.add_argument("--metric", choices=["unweighted", "faith_pd"], default="unweighted", help="UniFrac metric type")
    parser.add_argument("--num-workers", type=int, default=None, help="Number of worker processes (default: CPU count)")
    parser.add_argument(
        "--threads-per-worker", type=int, default=4, help="Number of threads per worker for UniFrac computation"
    )
    parser.add_argument("--chunk-size", type=int, default=1000, help="Number of samples per chunk")
    parser.add_argument("--rarefy-depth", type=int, default=None, help="Rarefaction depth")
    parser.add_argument("--rarefy-seed", type=int, default=None, help="Rarefaction random seed")
